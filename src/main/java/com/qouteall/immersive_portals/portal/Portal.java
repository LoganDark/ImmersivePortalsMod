...
public class Portal extends Entity {
    ...
    public final Vec3d untransformPointRough(Vec3d pos) {
        Vec3d offset = getPos().subtract(destination);
        return pos.add(offset);
    }
    ...
    public Vec3d untransformPoint(Vec3d pos) {
        if (rotation == null) {
            return untransformPointRough(pos);
        }

        Vec3d localPos = pos.subtract(destination);

        return untransformLocalVec(localPos).add(getPos());
    }
    ...
    private Box boxContainingPoints(List<Vec3d> points) {
        if (points.isEmpty()) {
        ...
        AtomicDouble xMin = new AtomicDouble(first.x);
        AtomicDouble xMax = new AtomicDouble(first.x);
        AtomicDouble yMin = new AtomicDouble(first.y);
        AtomicDouble yMax = new AtomicDouble(first.y);
        AtomicDouble zMin = new AtomicDouble(first.z);
        AtomicDouble zMax = new AtomicDouble(first.z);

        points.iterator().forEachRemaining(point -> {
            xMin.set(Math.min(xMin.get(), point.x));
            xMax.set(Math.max(xMax.get(), point.x));
            yMin.set(Math.min(yMin.get(), point.y));
            yMax.set(Math.max(yMax.get(), point.y));
            zMin.set(Math.min(zMin.get(), point.z));
            zMax.set(Math.max(zMax.get(), point.z));
        });

        return new Box(
            new Vec3d(xMin.get(), yMin.get(), zMin.get()),
            new Vec3d(xMax.get(), yMax.get(), zMax.get())
        );
    }

    public Box transformBox(Box box) {
        double xMin = Math.min(box.x1, box.x2);
        double xMax = Math.max(box.x1, box.x2);
        double yMin = Math.min(box.y1, box.y2);
        double yMax = Math.max(box.y1, box.y2);
        double zMin = Math.min(box.z1, box.z2);
        double zMax = Math.max(box.z1, box.z2);

        Vec3d tlf = new Vec3d(xMin, yMin, zMin);
        Vec3d trf = new Vec3d(xMax, yMin, zMin);
        Vec3d tlb = new Vec3d(xMin, yMin, zMax);
        Vec3d trb = new Vec3d(xMax, yMin, zMax);
        Vec3d blf = new Vec3d(xMin, yMax, zMin);
        Vec3d brf = new Vec3d(xMax, yMax, zMin);
        Vec3d blb = new Vec3d(xMin, yMax, zMax);
        Vec3d brb = new Vec3d(xMax, yMax, zMax);

        Vec3d tlfTransformed = transformPoint(tlf);
        Vec3d trfTransformed = transformPoint(trf);
        Vec3d tlbTransformed = transformPoint(tlb);
        Vec3d trbTransformed = transformPoint(trb);
        Vec3d blfTransformed = transformPoint(blf);
        Vec3d brfTransformed = transformPoint(brf);
        Vec3d blbTransformed = transformPoint(blb);
        Vec3d brbTransformed = transformPoint(brb);

        return boxContainingPoints(Lists.newArrayList(
            tlfTransformed,
            trfTransformed,
            tlbTransformed,
            trbTransformed,
